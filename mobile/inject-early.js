...
(function() {
  ...
  // MOBILE-COMPAT: auth gate REMOVED. On desktop the background SW writes
  // `ofStatsAuthStatus` to localStorage before any content script runs (via
  // broadcastAuthStatus). On mobile (Orion), the SW often never wakes, so the
  // flag is set later by the popup — but by then this script already
  // early-returned and missed every observer/listener registration.
  // Subscription enforcement still happens inside applyModifications via the
  // server-validated `ofStatsSubscriptionActive` chrome.storage flag.
  ...
  function drawMonthChartEarly(canvas, row) {
    ...
    // Set canvas dimensions — match parent width so on narrow viewports (mobile)
    // the chart doesn't overflow / clip. Falls back to 608px when parent width
    // isn't measurable yet. High-DPI: backing store sized for retina, CSS size
    // logical (drawing code reads clientWidth/Height and pre-scales ctx by dpr).
    var parentEl = canvas.parentElement;
    var parentWidth = (parentEl && parentEl.getBoundingClientRect && parentEl.getBoundingClientRect().width) || 608;
    if (parentWidth < 100) parentWidth = 608;
    parentWidth = Math.round(parentWidth);
    var dprM = window.devicePixelRatio || 1;
    canvas.width = Math.round(parentWidth * dprM);
    canvas.height = Math.round(220 * dprM);
  ...
  function drawMonthChartCanvas(canvas, chartData) {
    ...
    // Use CSS (logical) dimensions for layout math. Backing store is dpr times
    // larger, so we scale the context once and then draw in CSS units. This
    // gives crisp rendering on retina mobile (otherwise canvas looks blurry).
    ...
    function animate(currentTime) {
      ...
      // CLUSTER COMPACTNESS layout — user-validated values via canvas-demo.html:
      //   firstLabelOffset=10  pos[0] inset from left edge
      //   lastPairGap=0        pos[3] glued to pos[4] (right-most)
      //   compactness=16%      slight rightward shift of pos[0..3] toward pos[3]
      // Then pos[1] and pos[2] uniformly distributed between pos[0] and pos[3]
      // so VISUAL gaps between labels are identical despite different widths.
      var firstLabelOffset = 10;
      var lastPairGap = 0;
      var compactness = 16;
      var positions;
      if (canvas._fixedDatePositions) {
        ...
        if (positions[0] < pos0Base) positions[0] = pos0Base;
      ...
      labelsToUse.forEach(function(label, i) {
        ...
        ctx.textAlign = (i === labelsToUse.length - 1) ? 'right' : 'left';
  ...
  function drawMonthChartCanvasNoAnimation(canvas, chartData) {
    ...
    // High-DPI scaling — see drawMonthChartCanvas for explanation.
    var dpr = window.devicePixelRatio || 1;
    var width = canvas.clientWidth || Math.round(canvas.width / dpr);
    var height = canvas.clientHeight || Math.round(canvas.height / dpr);
    ctx.setTransform(1, 0, 0, 1, 0, 0);
    ...
    // === РќРђРЎРўР РћР™РљРђ ===
    // CLUSTER COMPACTNESS layout — see animated version in this file for docs.
    var labelData = labelIndices.map(function(idx, i) {
      var label = chartData.labels[idx] || '';
      var dataPointX = padding.left + idx * xStep;
      var labelWidth = ctx.measureText(label).width;
    ...
    labelsToUse.forEach(function(label, i) {
      var x = Math.round(positions[i]);
      ctx.textAlign = (i === labelsToUse.length - 1) ? 'right' : 'left';
      ctx.fillText(label, x, Math.round(labelY));
    });
  }
  ...
  function getChartLabelColor() {
    // Pure black for crisp text on mobile (#333 looked muddy when subpixel-rendered)
    return isDarkMode() ? '#e8e8e8' : '#000000';
  }

  // Explicit `400` weight — without it WebKit canvas API picks system default
  // (~500) which looks bolder than OF native. Match production demo values.
  ...
  function createOverlayChart(originalCanvas, chartData) {
    ...
    // Create overlay canvas — match wrapper width so on narrow viewports (mobile)
    // the chart doesn't overflow / clip. Falls back to 608px when wrapper width
    // isn't measurable yet.
    // High-DPI fix: backing store is dpr times the CSS size so retina mobile
    // gets crisp rendering instead of blurry pixels. Drawing code reads CSS
    // size via clientWidth/Height and the 2D context is pre-scaled by dpr.
    var wrapperWidth = (wrapper && wrapper.getBoundingClientRect && wrapper.getBoundingClientRect().width) || 608;
    if (wrapperWidth < 100) wrapperWidth = 608;
    wrapperWidth = Math.round(wrapperWidth);
    var dpr = window.devicePixelRatio || 1;
    var canvas = document.createElement('canvas');
    canvas.width = Math.round(wrapperWidth * dpr);
    canvas.height = Math.round(200 * dpr);
    canvas.style.cssText = 'position: absolute; top: 0; left: 0; display: block; box-sizing: border-box; width: ' + wrapperWidth + 'px; height: 200px; z-index: 1;';
  ...
  function getSelectedStatisticsPeriod() {
    ...
    // MOBILE-COMPAT: on mobile OF the period dropdown DOM is structured differently
    // and we end up matching some random text via the fallback scan (e.g. "Last reply"
    // somewhere on the page). When the matched text doesn't fit either pattern, default
    // to 'last30' instead of 'other' — otherwise applyStatisticsEarningsPage bails out
    // and the chart/transactions never get built on mobile.
    return 'last30';
  }
