...
// worker never wakes up. To make auth work on mobile, the popup does fetch()
// calls DIRECTLY instead of round-tripping through the SW. The same SW path
// still exists for PC compatibility, but `mobileApiFetch` bypasses it.
const MOBILE_API_URL = 'https://stats-editor-production.up.railway.app/api';

// Direct popup-side fetch with auth header (token from chrome.storage). No
// dependency on the background service worker — works on Orion mobile.
async function mobileApiFetch(path, options = {}) {
  ...
  return fetch(MOBILE_API_URL + path, options);
}

// Direct login — no service worker round-trip. Mirrors background.js apiLogin.
async function mobileLogin(email, password) {
  try {
    ...
    return { success: false, error: 'Network error: ' + (error && error.message ? error.message : 'unknown') };
  }
}

// Direct register
async function mobileRegister(email, password) {
  try {
    const response = await fetch(MOBILE_API_URL + '/auth/register', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email: email, password: password })
    });
...
// Direct verifyAuth
async function mobileVerifyAuth() {
  try {
    const response = await mobileApiFetch('/auth/verify');
...
// Replace chrome.scripting.executeScript with chrome.tabs.sendMessage to
// content.js, which exposes a `pageLocalStorage` action. chrome.scripting
// is unreliable in mobile browsers (Orion in particular) and causes the
// Apply button to hang indefinitely.
//
// CRITICAL: every sendMessage call is wrapped in a 3s timeout. On mobile, if
// the content script hasn't loaded (or is sleeping), sendMessage can hang
// FOREVER — even with try/catch. This froze initAuth on popup re-open. With
// timeout, helpers return null gracefully and init continues.
...
async function pageStorageSet(tabId, key, value) {
  try {
    return await _withTimeout(
      chrome.tabs.sendMessage(tabId, { action: 'pageLocalStorage', op: 'set', key: key, value: value }),
      3000
    );
  } catch (e) { return null; }
}
async function pageStorageRemove(tabId, key) {
  try {
    return await _withTimeout(
      chrome.tabs.sendMessage(tabId, { action: 'pageLocalStorage', op: 'remove', key: key }),
      3000
    );
  } catch (e) { return null; }
}
async function pageStorageGet(tabId, key) {
  try {
    const resp = await _withTimeout(
      chrome.tabs.sendMessage(tabId, { action: 'pageLocalStorage', op: 'get', key: key }),
      3000
    );
    return (resp && resp.success) ? resp.value : null;
  } catch (e) { return null; }
}
async function pageStorageBatch(tabId, ops) {
  try {
    return await _withTimeout(
      chrome.tabs.sendMessage(tabId, { action: 'pageLocalStorage', op: 'batch', ops: ops }),
      3000
    );
  } catch (e) { return null; }
}
...
async function initAuth() {
  ...
  try {
    ...
    // MOBILE-COMPAT: verify auth directly via popup-side fetch (bypassing the
    // service worker which is unreliable on Orion).
    let verifyResult;
    try {
      verifyResult = await mobileVerifyAuth();
    } catch (e) {
    ...
    // CRITICAL on mobile: inject-early.js / content.js / page-interceptor.js
    // gate their entire functionality on `localStorage.ofStatsAuthStatus`. On
    // desktop the background's broadcastAuthStatus sets this. On mobile the
    // service worker often never wakes, so we MUST set it here directly via
    // the content-script message handler. Without this flag the scripts
    // early-return and nothing applies, no matter what settings are saved.
    try {
      const tabs = await chrome.tabs.query({ url: '*://*.onlyfans.com/*' });
      for (const tab of tabs) {
        await pageStorageBatch(tab.id, [
          { op: 'set', key: 'ofStatsSubActive', value: 'true' },
          { op: 'set', key: 'ofStatsAuthStatus', value: 'authenticated' }
        ]);
      }
    } catch (e) {}
...
async function disablePluginDueToExpiredSubscription() {
  ...
  try {
    ...
    for (const tab of tabs) {
      try {
        // Set localStorage flags via content-script message (mobile-compat)
        await pageStorageBatch(tab.id, [
          { op: 'set', key: 'ofStatsSubActive', value: 'false' },
          { op: 'remove', key: 'ofStatsCache' },
          { op: 'remove', key: 'ofStatsEarningStats' },
          { op: 'remove', key: 'ofStatsEarningsData' }
        ]);
    ...
    try {
      ...
      for (const tab of tabs) {
        try {
          await pageStorageBatch(tab.id, [
            { op: 'set', key: 'ofStatsSubActive', value: 'true' },
            { op: 'set', key: 'ofStatsCache', value: settingsJSON }
          ]);
...
// Start payment status checking - auto-check every 15 seconds
function startPaymentStatusCheck(paymentId) {
  ...
  // Start auto-checking every 15 seconds immediately
  paymentAutoCheckInterval = setInterval(() => checkPaymentOnce(paymentId, true), 15000);
  
  // Also set up manual check button (speeds up to every 10s on click)
  const checkBtn = document.getElementById('checkPaymentBtn');
  if (checkBtn) {
...
function startPaymentTimer(expiresAt) {
  ...
  const updateTimer = () => {
    ...
    if (diff <= 0) {
      // Timer expired - but payment may still confirm later
      clearInterval(paymentTimerInterval);
      timeLeftEl.textContent = 'Expired';
      timerEl.classList.add('expired');
      // Don't redirect - keep auto-checking in background
      // Payment can still be confirmed after timer expires
      return;
    }
...
function setupAuthListeners() {
  ...
  const supportModalCancel = document.getElementById('supportModalCancel');
  if (supportModalClose) supportModalClose.addEventListener('click', closeSupportModal);
  if (supportModalCancel) supportModalCancel.addEventListener('click', closeSupportModal);
  if (supportModalOverlay) {
    supportModalOverlay.addEventListener('click', (e) => {
      if (e.target === supportModalOverlay) closeSupportModal();
    });
  }
  ...
  if (supportModalSend) {
    supportModalSend.addEventListener('click', async () => {
      ...
      supportModalSend.disabled = true;
      supportModalSend.innerHTML = `<svg viewBox="0 0 24 24" fill="none" width="14" height="14" style="animation:spin 1s linear infinite"><circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="2" stroke-dasharray="30 10" /></svg> Sending...`;
      ...
      return; // don't call closeSupportModal again
    });
  }
...
async function addNotification(type, title, message) {
  ...
  if (btn) {
    btn.classList.remove('new-notification');
    // Force reflow to restart animation
    void btn.offsetWidth;
    btn.classList.add('new-notification');
  }
...
async function handleLogin() {
  ...
  try {
    // MOBILE-COMPAT: bypass service worker entirely — popup makes the auth
    // fetch directly. Orion mobile service workers often don't wake up,
    // causing chrome.runtime.sendMessage to hang. Direct fetch always works.
    const result = await mobileLogin(emailInput.value, passwordInput.value);
    ...
    if (result.success) {
      ...
      // CRITICAL on mobile: set ofStatsAuthStatus on all OnlyFans tabs so
      // inject-early.js / content.js / page-interceptor.js stop early-returning.
      // (See full explanation in initAuth's same block.)
      try {
        const tabs = await chrome.tabs.query({ url: '*://*.onlyfans.com/*' });
        for (const tab of tabs) {
          await pageStorageBatch(tab.id, [
...
async function handleRegister() {
  ...
  try {
    // MOBILE-COMPAT: direct popup-side fetch instead of service-worker round-trip
    const result = await mobileRegister(registerEmail.value, registerPassword.value);
...
async function handleLogout() {
  ...
  // Clear presets (they belong to the account, not the browser)
  currentPresets = {};
  activePresetName = '';
  
  // Clear saved popup state, subscription status, and presets
  await chrome.storage.local.remove(['popupScreen', 'popupPendingEmail', 'popupPaymentId', 'popupSelectedPlan', 'popupPayAddress', 'popupPaymentExpires', 'ofStatsSubscription', 'ofStatsSubscriptionActive', 'ofStatsPresets', 'ofStatsActivePreset']);
  ...
  try {
    ...
    for (const tab of tabs) {
      await pageStorageBatch(tab.id, [
        { op: 'remove', key: 'ofStatsSubActive' },
        { op: 'remove', key: 'ofStatsCache' },
        { op: 'remove', key: 'ofStatsEarningStats' },
        { op: 'remove', key: 'ofStatsEarningsData' }
      ]);
  ...
  try {
    ...
    try {
      ...
      if (tab && tab.url && tab.url.includes('onlyfans.com')) {
        // MOBILE: replaced chrome.scripting.executeScript with a dedicated
        // content-script message that runs the same conditional logic.
        try {
          await chrome.tabs.sendMessage(tab.id, {
            action: 'saveSettingsToCache',
            settingsJSON: JSON.stringify(settings),
            shouldClearEarnings: earningsChanged,
            shouldClearBalance: balanceChangedToNewValue,
            preserveEarnings: preserveEarningStats
          });
        } catch (e) {}
      }
    } catch (e) {
      ...
      if (response) {
        if (response.modelName) {
          ...
          if (currentSettings.ofStatsSettings) {
            ...
            // Update localStorage cache immediately on ALL tabs via content-script message
            try {
              const allTabs = await chrome.tabs.query({ url: '*://*.onlyfans.com/*' });
              for (const t of allTabs) {
                try {
                  await chrome.tabs.sendMessage(t.id, { action: 'updateCachedUsername', username: cleanUsername });
                } catch (e) {}
              }
            } catch (e) {
...
async function applyChanges() {
  ...
  // MOBILE: clear reset/disabled flags via content-script message (no chrome.scripting)
  try {
    const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
    if (tab && tab.url && tab.url.includes('onlyfans.com')) {
      await pageStorageBatch(tab.id, [
        { op: 'remove', key: 'ofStatsResetPending' },
        { op: 'remove', key: 'ofStatsStatisticsChartDisabled' }
      ]);
    }
  } catch (e) {
  ...
  try {
    ...
    try {
      ...
      // MOBILE: write ofStatsCache to ALL OnlyFans tabs via content-script message
      // (chrome.scripting.executeScript is unreliable on mobile and hangs Apply).
      const settingsJSON = JSON.stringify(settings);
      for (const t of allTabs) {
        await pageStorageSet(t.id, 'ofStatsCache', settingsJSON);
      }
      ...
      // Reload all tabs that need updating. On mobile we DON'T wait for the
      // reload to finish via chrome.tabs.onUpdated — that listener is unreliable
      // in mobile browsers (Orion in particular) and causes the spinner to
      // hang forever. Fire-and-forget reload, then clean up UI immediately.
      for (const t of tabsToReload) {
        try { chrome.tabs.reload(t.id); } catch (e) {}
      }

      // Clean up UI right away — user sees the reload happen in the tab.
...
async function resetSettings() {
  ...
  try {
    const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
    if (tab && tab.url && tab.url.includes('onlyfans.com')) {
      // MOBILE: full Reset cleanup is now done by content-script handler
      try {
        await chrome.tabs.sendMessage(tab.id, { action: 'resetPageState' });
      } catch (e) {}
    }
  } catch (e) {
...
function setupEventListeners() {
  ...
  // MOBILE DEBUG: Capture DOM button — sends a captureDiagnostic message to
  // the active OF tab and dumps the result into a readonly textarea. Used to
  // gather mobile DOM samples so we can adapt selectors.
  const captureDomBtn = document.getElementById('captureDomBtn');
  const captureDomOutput = document.getElementById('captureDomOutput');
  if (captureDomBtn && captureDomOutput) {
    captureDomBtn.addEventListener('click', async () => {
      captureDomBtn.disabled = true;
      captureDomBtn.textContent = '⏳ Capturing...';
      captureDomOutput.style.display = 'block';
      captureDomOutput.value = 'Querying active OnlyFans tab...';
      try {
        const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
        if (!tab || !tab.url || !tab.url.includes('onlyfans.com')) {
          captureDomOutput.value = 'ERROR: active tab is not onlyfans.com — open OF in a tab first.';
          return;
        }
        const resp = await chrome.tabs.sendMessage(tab.id, { action: 'captureDiagnostic' });
        if (resp && resp.success && resp.dump) {
          captureDomOutput.value = resp.dump;
        } else {
          captureDomOutput.value = 'ERROR: no response from content script. Reload OF page and try again.\nResponse: ' + JSON.stringify(resp);
        }
      } catch (e) {
        captureDomOutput.value = 'EXCEPTION: ' + (e && e.message ? e.message : String(e));
      } finally {
        captureDomBtn.disabled = false;
        captureDomBtn.textContent = '🔍 Capture DOM (debug)';
      }
    });
  }
...
// Sync presets FROM server (server is always source of truth per account)
async function syncPresetsFromServer() {
  try {
    ...
    // Server is always source of truth — each account has its own presets
    ...
    if (serverCount > 0) {
      ...
      log('OF Stats: No presets on server for this account');
    }
  } catch (error) {
...
async function getEarningStatsFromPage() {
  try {
    const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
    if (tab && tab.url && tab.url.includes('onlyfans.com')) {
      const resp = await chrome.tabs.sendMessage(tab.id, { action: 'getEarningStats' });
      return (resp && resp.success) ? resp.value : null;
    }
  } catch (e) {
  ...
  try {
    ...
    if (tab && tab.url && tab.url.includes('onlyfans.com')) {
      const resp = await chrome.tabs.sendMessage(tab.id, {
        action: 'setEarningStats',
        data: earningStatsData
      });
      if (resp && resp.success && resp.verified) {
...
async function clearEarningStatsFromPage() {
  try {
    const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
    if (tab && tab.url && tab.url.includes('onlyfans.com')) {
      await chrome.tabs.sendMessage(tab.id, { action: 'clearEarningStats' });
    }
  } catch (e) {
...
async function clearTransactionDataFromPage() {
  try {
    ...
    if (tab && tab.url && tab.url.includes('onlyfans.com')) {
      await pageStorageBatch(tab.id, [
        { op: 'remove', key: 'ofStatsEarningsData' },
        { op: 'remove', key: 'ofStatsEarningsKey' }
      ]);
    }
  } catch (e) {
