...
(function() {
  ...
  // MOBILE-COMPAT: auth gate REMOVED. See inject-early.js for the explanation —
  // on Orion the SW never writes ofStatsAuthStatus before content scripts run,
  // so this gate would always early-return and the whole extension would be dead.
  // applyModifications still calls isSubscriptionActive() before applying fake
  // values, so unauthenticated/expired users still don't get any modification.
  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    try {
      switch (request.action) {
        ...
        // MOBILE-COMPAT: handles the saveSettings flow that previously used
        // chrome.scripting.executeScript with page-context conditional logic
        // (reading ofStatsEarningStats, checking fromPreset, removing attrs
        // from .b-stats-wrap). Encapsulating it in content.js lets the popup
        // fire one message instead of injecting code.
        // MOBILE-DIAGNOSTIC: capture HTML of key elements so we can adapt
        // selectors for mobile OF DOM. Triggered by the "Capture DOM" button
        // in popup. Returns the result as a single text blob the user can
        // screenshot.
        case 'captureDiagnostic':
          try {
            ...
            function snap(label, selector, maxLen) {
              maxLen = maxLen || 400;
              var nodes = document.querySelectorAll(selector);
              lines.push('### ' + label + ' [' + selector + '] count=' + nodes.length);
            ...
            function snapText(label, selector) {
              var nodes = document.querySelectorAll(selector);
              lines.push('### ' + label + ' [' + selector + '] count=' + nodes.length);
              for (var i = 0; i < Math.min(nodes.length, 5); i++) {
                var txt = (nodes[i].textContent || '').trim().substring(0, 120);
                var cls = nodes[i].className || '';
                lines.push('  [' + i + '] cls="' + (typeof cls === 'string' ? cls : cls.baseVal || '') + '" text="' + txt + '"');
              }
              lines.push('');
            }

            // Page wrapper markers
            snap('Statistics page wrapper', '.b-statistics-page-content__wrapper');

            // Chart containers
            snap('Chart elements', '.b-chart', 800);
            snap('Chart wrappers', '.b-chart__wrapper');
            snap('Canvas elements', 'canvas');
            snap('Stats wrap', '.b-stats-wrap');

            // Earnings statistics
            snap('Useful data blocks', '.b-useful-data');
            snap('Elements determinant', '.b-elements-determinant');

            // Balance + transactions
            snap('Balance elements', '[class*="balance"]', 300);
            snapText('Tip/Transaction rows', 'tr.m-pending,tr.m-complete,tr[class*="earn"]');
            snap('Earnings table', 'table.b-table.m-earnings', 600);

            // Top creators banner
            snap('Top rated', '.b-top-rated', 400);
            snap('Top creators ours', '#of-stats-top-creators-rated');

            // Stats categories
            snapText('Stats row values', '.b-stats-row__val,.b-stats-row__total-net');

            // Sidebar / mobile menu
            snap('Sidebar user data', '.l-sidebar__user-data__item');
            snap('Mobile menu user data', '.m-native-custom-menu__user-title');
            snap('User name elements', '.l-sidebar__user-name,.g-user-username');

            // Engagement summary (subs page)
            snap('Engagements summary', '.b-engagements-summary__item');

            // Our generated elements
            snap('Our generated', '[data-of-stats-generated],[data-of-stats-subs-processed],[data-of-stats-applied]', 200);

            sendResponse({ success: true, dump: lines.join('\n') });
          } catch (e) {
            sendResponse({ success: false, error: String(e) });
          }
          break;

        // MOBILE-COMPAT: read EarningStats (preset workflow)
        case 'getEarningStats':
          try {
            var raw = localStorage.getItem('ofStatsEarningStats');
            sendResponse({ success: true, value: raw ? JSON.parse(raw) : null });
          } catch (e) {
            sendResponse({ success: false, error: String(e) });
          }
          break;

        // MOBILE-COMPAT: write EarningStats from a preset
        case 'setEarningStats':
          try {
            ...
            if (data) {
              ...
              if (data.months && data.months.length > 0) {
                var oldestMonth = data.months[data.months.length - 1];
                data.oldestMonth = { year: oldestMonth.year, month: oldestMonth.month };
              }
              ...
              var c1 = document.querySelector('.b-stats-wrap');
              if (c1) {
                c1.removeAttribute('data-of-stats-months-applied');
                c1.removeAttribute('data-of-stats-months-replaced');
              }
              var verified = localStorage.getItem('ofStatsEarningStats');
              var ok = verified ? JSON.parse(verified).fromPreset === true : false;
              sendResponse({ success: true, verified: ok });
            } else {
        ...
        // MOBILE-COMPAT: clear EarningStats (preset reset)
        case 'clearEarningStats':
          try {
            localStorage.removeItem('ofStatsEarningStats');
            localStorage.removeItem('ofStatsChartDataCache');
            localStorage.removeItem('ofStatsChartGrossValue');
            localStorage.removeItem('ofStatsEarningsBreakdownCache');
            var c2 = document.querySelector('.b-stats-wrap');
            if (c2) {
              c2.removeAttribute('data-of-stats-months-applied');
              c2.removeAttribute('data-of-stats-months-replaced');
            }
        ...
        // MOBILE-COMPAT: full Reset-button page cleanup that previously ran via
        // chrome.scripting.executeScript with DOM manipulation + chart destroys.
        case 'resetPageState':
          try {
            localStorage.setItem('ofStatsStatisticsChartDisabled', 'true');
            localStorage.removeItem('ofStatsEarningsData');
            localStorage.removeItem('ofStatsEarningsKey');
            var bsContainer = document.querySelector('.b-stats-wrap');
            if (bsContainer) {
              bsContainer.removeAttribute('data-of-stats-months-applied');
              bsContainer.removeAttribute('data-of-stats-months-replaced');
            }
            if (typeof window.stopOriginalElementsObserver === 'function') {
              try { window.stopOriginalElementsObserver(); } catch (e) {}
            ...
            if (statisticsWrapper) {
              ...
              if (typeof Chart !== 'undefined') {
                try { if (mainCanvas) { var mC = Chart.getChart(mainCanvas); if (mC) mC.destroy(); } } catch (e) {}
                try { if (asideCanvas) { var aC = Chart.getChart(asideCanvas); if (aC) aC.destroy(); } } catch (e) {}
                document.querySelectorAll('canvas[id^="of-stats-mini-chart-"]').forEach(function (cv) {
                  try { var ch = Chart.getChart(cv); if (ch) ch.destroy(); } catch (e) {}
                });
              }
              statisticsWrapper.querySelectorAll('[data-of-stats-generated]').forEach(function (el) { el.remove(); });
            ...
            if (hideEarningsStyle) hideEarningsStyle.remove();
            sendResponse({ success: true });
          } catch (e) {
            sendResponse({ success: false, error: String(e) });
          }
        ...
        // MOBILE-COMPAT: update myModelUsername inside the cached settings JSON
        case 'updateCachedUsername':
          try {
            var cached = localStorage.getItem('ofStatsCache');
            if (cached) {
              var s = JSON.parse(cached);
              s.myModelUsername = request.username;
              localStorage.setItem('ofStatsCache', JSON.stringify(s));
            }
            sendResponse({ success: true });
          } catch (e) {
            sendResponse({ success: false, error: String(e) });
        ...
        case 'saveSettingsToCache':
          try {
            localStorage.setItem('ofStatsCache', request.settingsJSON);
            if (request.preserveEarnings) {
              sendResponse({ success: true, action: 'preserved' });
              break;
            }
            ...
            function clearMonthsAppliedFlag() {
            ...
            if (request.shouldClearEarnings && !isFromPreset) {
              localStorage.removeItem('ofStatsEarningsData');
              localStorage.removeItem('ofStatsEarningsKey');
              localStorage.removeItem('ofStatsEarningStats');
              localStorage.removeItem('ofStatsChartDataCache');
              localStorage.removeItem('ofStatsChartGrossValue');
              localStorage.removeItem('ofStatsEarningsBreakdownCache');
              clearMonthsAppliedFlag();
            }
            if (request.shouldClearBalance && !request.shouldClearEarnings && !isFromPreset) {
              localStorage.removeItem('ofStatsEarningStats');
              localStorage.removeItem('ofStatsChartDataCache');
              localStorage.removeItem('ofStatsChartGrossValue');
              localStorage.removeItem('ofStatsEarningsBreakdownCache');
        ...
        // MOBILE-COMPAT: chrome.scripting.executeScript is unreliable in mobile
        // browsers (Orion on iOS in particular). Popup uses this message to
        // read/write/remove localStorage on the page instead of injecting code.
        // Operations:
        //   { action: 'pageLocalStorage', op: 'set', key, value }
        //   { action: 'pageLocalStorage', op: 'get', key }
        //   { action: 'pageLocalStorage', op: 'remove', key }
        //   { action: 'pageLocalStorage', op: 'batch', ops: [{op,key,value}, ...] }
        case 'pageLocalStorage':
          try {
            if (request.op === 'set') {
              localStorage.setItem(request.key, String(request.value));
              sendResponse({ success: true });
            } else if (request.op === 'get') {
              var v = localStorage.getItem(request.key);
              sendResponse({ success: true, value: v });
            } else if (request.op === 'remove') {
              localStorage.removeItem(request.key);
              sendResponse({ success: true });
            } else if (request.op === 'batch' && Array.isArray(request.ops)) {
              request.ops.forEach(function (o) {
                try {
                  if (o.op === 'set') localStorage.setItem(o.key, String(o.value));
                  else if (o.op === 'remove') localStorage.removeItem(o.key);
                } catch (e) {}
              });
              sendResponse({ success: true });
            } else {
              sendResponse({ success: false, error: 'Unknown op: ' + request.op });
            }
          } catch (e) {
            sendResponse({ success: false, error: String(e) });
          }
