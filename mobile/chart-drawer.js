...
(function() {
  ...
  // MOBILE-COMPAT: auth gate REMOVED (see inject-early.js for explanation).
  ...
  window.addEventListener('of-stats-draw-statistics-charts', function(event) {
    ...
    var drawGridPlugin = {
      ...
      beforeDatasetsDraw: function(chart) {
        ...
        // MOBILE: ignore Chart.js gridLineItems (desktop-tuned with weird -90/+33
        // offsets that break on narrow viewports). Compute our own 5 grid X
        // positions: first one inset by `gridLeftOffset` from chartArea.left,
        // last one at chartArea.right, others evenly distributed.
        var gridLeftOffset = 28;
        var customGridLeft = chartArea.left + gridLeftOffset;
        var customGridRight = chartArea.right;
        var customGridXs = [];
        for (var __g = 0; __g < 5; __g++) {
          customGridXs.push(customGridLeft + (customGridRight - customGridLeft) * (__g / 4));
        }

        // For main chart: draw 5 vertical grid lines spanning chart top to
        // CANVAS BOTTOM (through padding.bottom). Without extending past
        // chartArea.bottom the lines visibly stop ABOVE the inter-canvas gap
        // and look disconnected from the aside chart's vertical lines below.
        // (Note: removed the horizontal separator line at chartArea.bottom + 8
        // — it doubled with aside chart's top border which looked like 2 lines.)
        if (isMainChart) {
          var __mainCanvasBottom = chart.height || (chartArea.bottom + 8);
          for (var __m = 0; __m < customGridXs.length; __m++) {
            var __mx = Math.round(customGridXs[__m]) + 0.5;
            ctx.beginPath();
            ctx.moveTo(__mx, chartArea.top);
            ctx.lineTo(__mx, __mainCanvasBottom);
            ctx.lineWidth = 1;
            ctx.strokeStyle = gridColorLight;
            ctx.stroke();
          }
        ...
        // For aside chart: draw 5 vertical lines from CANVAS TOP (0, through
        // padding.top) all the way down through padding.bottom (date area) to
        // chart.height. Extending past chartArea.top/bottom into padding makes
        // the lines visually continuous with the main chart's lines above.
        // All 5 date labels are LEFT-aligned to their grid line — including
        // the last one (no special right-align case, matches OF native mobile).
        if (isAsideChart) {
          // Stop vertical lines just below the date labels (line2 baseline + a few px),
          // NOT at canvas bottom — extending all the way through padding.bottom
          // leaves a visible "tail" below the dates.
          var __asideLineBottom = chartArea.bottom + 34;

          for (var __a = 0; __a < customGridXs.length; __a++) {
            var __ax = Math.round(customGridXs[__a]) + 0.5;
            ctx.beginPath();
            ctx.moveTo(__ax, 0);
            ctx.lineTo(__ax, __asideLineBottom);
            ctx.lineWidth = 1;
            ctx.strokeStyle = gridColorLight;
            ctx.stroke();
          }

          // Date labels — all left-aligned to grid lines (no right-align hack)
          ...
          if (monthlyMode) {
            ...
            for (var __mi = 0; __mi < customGridXs.length; __mi++) {
              var __mLabelIdx = (__mi === customGridXs.length - 1)
                ? (totalLabels - 1)
                : Math.round((totalLabels - 1) * (__mi / 4));
              var __mLabel = labels[__mLabelIdx] || '';
              var __mParts = __mLabel.split(', ');
              var __mX = Math.round(customGridXs[__mi]) + 4;
              ctx.fillText((__mParts[0] || '') + ',', __mX, chartArea.bottom + 6);
              ctx.fillText(__mParts[1] || '', __mX, chartArea.bottom + 18);
            }
          } else {
            // Daily mode: 5 dates at day indices 0, 7, 14, 21, 29
            var __dailyIndices = [0, 7, 14, 21, 29];
            for (var __di = 0; __di < __dailyIndices.length; __di++) {
              var __dDate = new Date(startDate);
              __dDate.setDate(startDate.getDate() + __dailyIndices[__di]);
              var __dLine1 = __dDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) + ',';
              var __dLine2 = __dDate.getFullYear().toString();
              var __dX = Math.round(customGridXs[__di]) + 4;
              ctx.fillText(__dLine1, __dX, chartArea.bottom + 6);
              ctx.fillText(__dLine2, __dX, chartArea.bottom + 18);
            }
          }
        }
